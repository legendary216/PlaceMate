    ...
    export default function MentorHome() {
      ...
      const handleConnectionResponse = async (requestId, newStatus) => {
       // --- FIX IS HERE ---
      const actionId = `${requestId}-${newStatus}`;
...
                      // --- ⬇️ CHANGE IS HERE ⬇️ ---
                      // Only disable this button if IT is loading
                      disabled={actionLoading === `${req._id}-rejected`}
                  >
                      // This spinner logic was already correct
                      {actionLoading === `${req._id}-rejected` ? <ActivityIndicator size="small" color="#991b1b" /> : <X size={18} color="#991b1b" />}
                  </Pressable>
                  <Pressable 
...
                  >
                      // This spinner logic was already correct
                      {actionLoading === `${req._id}-accepted` ? <ActivityIndicator size="small" color="#166534" /> : <Check size={18} color="#166534" />}
                  </Pressable>
              </View>
            </View>
        ))}</View>);
